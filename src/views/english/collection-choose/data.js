...
const routePath = "/api/english/collection-word";
    ...
    render: (text, record) => (
      <div>
        {record.type === "choose" ? (
          <Button
            onClick={() =>
              chooseItem(record.collection_id, record.collection_word_id)
            }
          >
            添加
          </Button>
          ...
          <Button
            onClick={() =>
              replaceItem(record.collection_id, record.collection_word_id)
            }
          >
            替换
          </Button>
...
const replaceItem = (collection_id, collection_word_id) => {
  log(33, collection_id, collection_word_id);
  let params = { collection_id: collection_word_id };
  get(routePath, params).then(res => {
    if (res.data.data.length === 0) {
      message.info("item已存在");
    } else {
      post(routePath, params).then(res => {
        message.success("添加成功");
      });
    }
  });
};
const chooseItem = (collection_id, collection_word_id) => {
  log(333);
  return;
  let params = { collection_id: collection_word_id };
  get(routePath, params).then(res => {
